...
So this exercise wants me to manipulate the dom using a effect imported from a package.

I can access the dom node with `React.useRef` and using the `ref={}` property on the HTML element.

Anytime i need to interact with the DOM, that interaction is a side effect. I need to use `React.useEffect()` for that. An important thing I learned here is that without garbage collection, using an effect inside a component can lead to a memory leak as the dom could still exist in memory. So i need to return a cleanup function inside the `useEffect` function to prevent that from happening.

Lastly, I use an empty array list as a dependency to prevent the useEffect from being called on every single re-render. So it happens once when mounted.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=05%3A%20useRef%20and%20useEffect%3A%20DOM%20interaction&em=pricew86%40gmail.compricew86%40gmail.com).
