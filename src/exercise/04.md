...
For the extra credit (1) here the main challenge was remember how to implement useEffect for setting and getting local storage without using the provided util function. So, I went back to `src/exercise/02.js` to find an implementation and watched the solution again for that exercise. 

First step is to create a `React.useEffect()` function to `setItem()` and here i was setting the key with the string `squares` and passing the array `squares` to this key.

The second step was to use `getItem()` in the `React.useState()` inline function to determine if there were any items, return if there are or return default if there aren't.

The missing piece of the puzzle for me was to use `JSON.stringify()` on the squares array to set properly in local storage, then to use `JSON.parse()` in the `useState()` function to parse the string into a javascript object.

Lastly, i need to optimize setting the local storage only when the `squares` array is updated

For extra credit 3, I mostly watched Kent as he moved state management out of the board component and into the game itself. He added moves, history, and step
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=04%3A%20useState%3A%20tic%20tac%20toe&em=pricew86%40gmail.compricew86%40gmail.com).
